...
const ManageMediaDetails: React.FC<ManageMediaDetailsProps> = ({ media }) => {
    ...
    const { id, title, description, synopsis, director, image, status, buy_price, rent_price, year, like, unlike, genre, streamingPlatform, streamingLinks, review } = media;
    // const [statusItem, setStatusItem] = useState(status)
    useEffect(() => {
      if(media) setIsLoading(false);
